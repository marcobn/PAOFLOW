...
if double_grid:
	...
	for ispin in range(nspin):
        	for i in range(nawf):
                	for j in range(nawf):
                        	Hkauxp[i,j,:,:,:,ispin] = FFT.fftn(HRaux[i,j,:,:,:,ispin],(nk1p,nk2p,nk3p))
                        	if read_S and ispin == 0:
                                	Skauxp[i,j,:,:,:] = FFT.fftn(SRaux[i,j,:,:,:],(nk1p,nk2p,nk3p))
        ...
        for ispin in range(nspin):
                for i in range(nawf):
                        for j in range(nawf):
                                HRauxp[i,j,:,:,:,ispin] = FFT.ifftn(Hkauxp[i,j,:,:,:,ispin])
                                if read_S and ispin == 0:
                                        SRauxp[i,j,:,:,:] = FFT.ifftn(Skauxp[i,j,:,:,:])
...
#	SRaux = SRauxp
	...
	#HRaux[:,:,:,:,:,:] = HRauxp[:,:,:nk1,:nk2,:nk3:,:]
	#SRaux[:,:,:,:,:] = SRauxp[:,:,:nk1,:nk2,:nk3]
...
N_counter = np.zeros((nrtot),dtype=int)
...
for i in range(nk1):
        for j in range(nk2):
                for k in range(nk3):
                        ...
                        if i < nk1i and j < nk2i and k < nk3i:
