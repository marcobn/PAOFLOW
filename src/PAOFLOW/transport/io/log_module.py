...
def log_parallelization_info(chunks: int, items: str):
    log_rank0(f'Parallelization information: Each rank processes approximately {chunks} {items}.')
...
def log_summary(data: ConductorData) -> None:
    ...
    log_rank0(f'           ovp orthogonaliz. :     {data.atomic_proj.do_overlap_transformation}')
    log_rank0('  </INPUT>')
    log_rank0('')
