...
def read_QE_wfc(data_controller, ik, ispin):
  ...
  # compute overlap
  ovp = np.zeros((nbnd,nbnd),dtype=np.complex128)
  for n in range(nbnd):
    for m in range(nbnd):
  ...
  owfc = sci.linalg.solve(X.T, wfc) 
  
  wfc = np.array(wfc).astype(np.complex128) * scalef
...
def ortho_atwfc_k(atwfc_k):
  ...
  if True:
    X = scipy.linalg.sqrtm(ovp)
    oatwfc_k = sci.linalg.solve(X.T, atwfc_k)
  else:
