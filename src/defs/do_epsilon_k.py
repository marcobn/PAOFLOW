...
def do_epsilon_k(E_k,pksp,kq_wght,omega,delta,temp,ispin):
    ...
    epsik = np.zeros((3,3,ene.size),dtype=float)
    epsik_aux = np.zeros((3,3,ene.size,1),dtype=float)
    epsik_aux1 = np.zeros((3,3,ene.size,1),dtype=float)

    epsik_aux[:,:,:,0] = epsik_loop(ini_ik,end_ik,ene,E_k,pksp,kq_wght,omega,delta,temp,ispin)

    if rank == 0:
        epsik[:,:,:]=epsik_aux[:,:,:,0]
        for i in range(1,size):
            comm.Recv(epsik_aux1,ANY_SOURCE)
            epsi[:,:,:] += epsik_aux1[:,:,:,0]
    else:
        comm.Send(epsik_aux,0)
    epsik = comm.bcast(epsik)
