...
def build_raman_script(cfg):
    ...
    lines = []
    lines.append('#!/usr/bin/env python3')
    lines.append('"""PAOFLOW non-resonant (Placzek) Raman workflow (generated by paoflow_gen.py).')
    lines.append('')
    ...
    lines.append(
        'LASER_NM = {}   # excitation wavelength (nm); None -> no (wL-wv)^4 factor'.format(
            repr(laser) if laser else 'None'
        )
    )
    lines.append(
...
def build_raman_plot_script(cfg):
    """Assemble a plot.raman.py that plots the broadened Raman spectrum."""
    lines = []
    lines.append('#!/usr/bin/env python3')
    ...
    lines.append('Restrict the frequency axis or save to a file with:')
    lines.append('')
    lines.append('    python plot.raman.py --xmin 0 --xmax 600 --save raman.png')
    lines.append('"""')
    lines.append('')
    lines.append('import argparse')
    lines.append('import os')
    ...
    lines.append('    args = parser.parse_args()')
    lines.append('')
    lines.append("    spectrum = _path('_raman_spectrum.dat')")
    lines.append('    if not os.path.isfile(spectrum):')
    lines.append("        print('{} not found; run main.raman.py first.'.format(spectrum))")
    lines.append('        return')
    lines.append("    modes = _path('_raman_modes.dat')")
    lines.append('')
    lines.append('    x_lim = None')
    lines.append('    if args.xmin is not None or args.xmax is not None:')
    lines.append('        x_lim = (args.xmin, args.xmax)')
    lines.append('')
    lines.append('    pplt.plot_raman_spectrum(')
    lines.append('        spectrum,')
    lines.append('        modes_file=modes if os.path.isfile(modes) else None,')
    lines.append("        title='Raman spectrum',")
    lines.append('        units=UNITS,')
    lines.append('        x_lim=x_lim,')
    lines.append('        filename=args.save,')
    lines.append('    )')
    lines.append('')
    lines.append('')
...
def collect_phonon(common):
    ...
    cfg['raman'] = ask_yes_no('Also generate a non-resonant Raman workflow (main.raman.py)?', False)
    if cfg['raman']:
        print('\nRaman (Placzek) finite-difference settings:')
