...
def build_phonon_plot_script(cfg):
    """Assemble a plot.phonon.py that plots the phonon dispersion and DOS.

    Reads the ``phonon_band.dat`` / ``phonon_dos.dat`` / ``phonon_band.labels``
    files written by ``main.phonon.py`` and renders them through
    :meth:`PAOFLOW.GPAO.GPAO.plot_phonons`.
    """
    lines = []
    lines.append('#!/usr/bin/env python3')
    lines.append('"""Plot the PAOFLOW phonon dispersion and DOS (generated by paoflow_gen.py).')
    lines.append('')
    ...
    lines.append('def main():')
    lines.append('    parser = argparse.ArgumentParser(')
    lines.append("        description='Plot the phonon dispersion and DOS.')")
    lines.append("    parser.add_argument('--ymin', type=float, default=None,")
