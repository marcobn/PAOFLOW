    ...
    with open(fname, "r") as f:
        ...
        try:
            ...
            while "celldm" not in lines[eL]:
                eL += 1
            celldm[:3] = [
                float(v) for i, v in enumerate(lines[eL].split()) if i % 2 == 1
            ]
            celldm[3:] = [
                float(v) for i, v in enumerate(lines[eL + 1].split()) if i % 2 == 1
            ]
...
def read_relaxed_coordinates_QE(fname: str):
    ...
    with open(fname, "r") as f:
        ...
        try:
            ...
            print(
                "WARNING: No atomic positions or cell coordinates were found.",
                flush=True,
            )
            raise e
...
def struct_from_inputfile_QE(fname: str) -> dict:
    ...
    for match in matches:
        match = match.replace(",@", "@")
        match = [
            s.replace(" ", "").split("!")[0] for s in re.split(", |@", match) if s != ""
        ]
...
def create_acbn0_inputfile(
    ...
    configuration : str or dict, optional
        Projection-basis preset (``'minimal'``, ``'standard'`` or
        ``'extended'``) or an explicit ``{element: [shell_labels]}``
        mapping forwarded to ``projections``.  A dict builds the
        all-electron basis from ``basispath`` (``internal=True``).
...
def _create_acbn0_local_basis_inputfile(
    ...
    if isinstance(configuration, dict):
        # Explicit {element: [shells]} mapping: build the all-electron
        # basis from ``basispath`` (requires ``internal=True``).
        proj_line = (
            f"paoflow.projections(internal=True, basispath={basis_arg}, "
