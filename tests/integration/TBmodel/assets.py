...
def _default_cache_dir() -> Path:
    ...
    if xdg:
        return Path(xdg).expanduser().resolve() / 'paoflow' / 'tbmodel-assets'
    return Path.home().expanduser().resolve() / '.cache' / 'paoflow' / 'tbmodel-assets'
...
def resolve_asset_source(
    *,
    ...
    """Resolve TBmodel asset configuration.

    Priority: explicit args -> env vars.

    Env vars:
      - PAOFLOW_TBMODEL_ASSET_ARCHIVE: local path to a tar.gz
      - PAOFLOW_TBMODEL_ASSET_URL: URL to download the tar.gz
      - PAOFLOW_TBMODEL_ASSET_SHA256: expected sha256 for the archive
      - PAOFLOW_TBMODEL_ASSET_VERSION: label used for caching (default: "dev")
    """

    env_archive = os.environ.get('PAOFLOW_TBMODEL_ASSET_ARCHIVE')
    env_url = os.environ.get('PAOFLOW_TBMODEL_ASSET_URL')
    env_sha = os.environ.get('PAOFLOW_TBMODEL_ASSET_SHA256')
    env_ver = os.environ.get('PAOFLOW_TBMODEL_ASSET_VERSION')
...
def ensure_assets_available(
    ...
    *,
    ...
    archive_path = cache / f'tbmodel_test_assets_{source.version}.tar.gz'
