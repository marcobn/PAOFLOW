...
def do_dos_calc(eig,emin,emax,delta,netot,v_kall,nk1,nk2,nk3,nawf,ispin):
    ...
    for ne in range(ene.size):
        for m in range(nawf):
            ...
            pdossum = np.zeros(1,dtype=float)
            aux = np.zeros(nsize,dtype=float)
            v_kaux = np.zeros((nsize,nawf,nawf),dtype=complex)
    
            comm.Barrier()
            comm.Scatter(eig,aux,root=0)
            comm.Scatter(v_kall,v_kaux,root=0)
            
            for n in range (nsize):
                pdosaux += 1.0/np.sqrt(np.pi)*np.exp(-((ene[ne]-aux[n])/delta)**2)/delta*np.real(np.matrix(v_kall[n,:,ispin])*np.matrix.getH(np.matrix(ve[n,:,:,ispin]))       
    
            comm.Barrier()
            comm.Reduce(pdosaux,pdossum,op=MPI.SUM)
            pdos[m,ne] = pdossum*float(nawf)/float(netot)
