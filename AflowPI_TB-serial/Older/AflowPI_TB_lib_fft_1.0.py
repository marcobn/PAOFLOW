...
def get_R_grid_fft(nk1,nk2,nk3,a_vectors):
	nrtot = nk1*nk2*nk3
	R = np.zeros((nrtot,3),dtype=float)
#	Kint = np.zeros((nrtot,3),dtype=float)
	R_wght = np.ones((nrtot),dtype=float)
	idx = np.zeros((nk1,nk2,nk3),dtype=int)
...
#                       	print('K',n,Rx,Ry,Rz,'||',Kint[n,0],Kint[n,1],Kint[n,2])
#                       	print('R',n,Rx,Ry,Rz,'||',R[n,0],R[n,1],R[n,2])

# Add the whole mesh using inversion symmetry (H(-k)=H(k))?
	return(R,R_wght,nrtot,idx)
