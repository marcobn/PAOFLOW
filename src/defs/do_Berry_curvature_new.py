...
def do_Berry_curvature(E_k,pksp,delta,temp,ibrav,alat,a_vectors,b_vectors,dkres,iswitch,nthread,npool):
    #----------------------
    ...
    if rank == 0:
        nk1,nk2,nk3,_,nawf,nawf,nspin = pksp.shape
    ...
    for pool in range(npool):
        ...
        if rank == 0:
            pksp_split = np.array_split(np.reshape(pksp,(nk1*nk2*nk3,3,nawf,nawf,nspin),order='C'),npool,axis=0)[pool]
