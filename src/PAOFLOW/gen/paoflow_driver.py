...
# --------------------------------------------------------------------------- #
# Electron-phonon workflow builder
# --------------------------------------------------------------------------- #
def build_elphon_script(cfg):
    """Assemble a main.elphon.py electron-phonon workflow from the config.

    The generated script drives the finite-difference electron-phonon workflow
    in three phases (write displaced supercells -> run pw.x -> analyse), which
    can be invoked separately (handy on HPC schedulers).  The analyse phase
    finite-differences the PAO Hamiltonian to ``dV = dH/du``, folds it to the
    primitive-cell real-space tensor ``g_R`` and -- when Eliashberg output is
    requested -- combines it with the harmonic force constants (fc2) of a phonon
    calculation on the same supercell to give the isotropic ``alpha^2F(omega)``
    and ``lambda``.
    """
    sc = cfg['supercell']
    ...
    lines = []
    lines.append('#!/usr/bin/env python3')
    lines.append('"""PAOFLOW electron-phonon workflow (generated by paoflow_gen.py).')
    lines.append('')
    lines.append('Finite-difference electron-phonon coupling from the PAO Hamiltonian.  The')
    lines.append('workflow runs in three phases, which can be invoked separately:')
    lines.append('')
    lines.append('    python main.elphon.py generate   # write displaced-supercell QE inputs')
    lines.append('    python main.elphon.py run        # run pw.x on every displaced supercell')
    lines.append('    python main.elphon.py analyse    # dV -> g_R -> (optional) a2F / lambda')
    lines.append('')
    lines.append('``all`` (the default) runs the three phases in sequence:')
    lines.append('')
    lines.append('    python main.elphon.py')
    lines.append('')
    lines.append('The Eliashberg alpha^2F / lambda step reuses the harmonic force constants')
    lines.append('(fc2) of a phonon calculation on the SAME supercell.  Set PHONON_FORCE_SETS')
    lines.append('to an existing FORCE_SETS to skip the phonon SCF runs; otherwise the phonon')
    lines.append('supercells are written, run and harvested alongside the electron-phonon ones.')
    lines.append('On HPC, run the phases separately so pw.x can go through the scheduler.')
    lines.append('All lengths follow the phonopy QE convention (Bohr).')
    lines.append('"""')
    lines.append('')
    ...
    lines.append('PREFIX = {!r}'.format(cfg['prefix']))
    lines.append(
        'BASISPATH = os.path.join(HERE, {!r})   # local PAO basis directory'.format(cfg['basisdir'])
    )
    lines.append(_pp_dir_line(cfg))
    lines.append("ELPHON_DIR = 'elphon'   # sub-dir (under OUTPUTDIR) for the e-ph supercells")
    lines.append("PHONON_DIR = 'phonon'   # sub-dir (under OUTPUTDIR) for the fc2 supercells")
    lines.append('')
    lines.append('# Finite-displacement settings.')
    lines.append('SUPERCELL_MATRIX = {}   # scalar, length-3 or 3x3'.format(sc))
    lines.append('DISPLACEMENT = {}   # displacement amplitude (Bohr)'.format(cfg['displacement']))
    ...
    lines.append(
        "CONFIGURATION = {!r}   # PAO basis: 'minimal', 'standard' or 'extended'".format(
            cfg['configuration']
        )
    )
    lines.append('PTHR = {}   # projectability threshold'.format(cfg['pthr']))
    lines.append('SHIFT_TYPE = {}   # PAO Hamiltonian shift scheme'.format(cfg['shift_type']))
    lines.append(
        'PROJECT_GOOD_SUBSPACE = {}   # project dV onto the good-projectability subspace '
        '(removes the off-diagonal eta-completion artifact)'.format(
            bool(cfg['project_good_subspace'])
        )
    )
    lines.append('')
    lines.append('# HUBBARD card injected into the displaced-supercell inputs (on-site U only).')
    lines.append(_hubbard_file_line(cfg))
    lines.append('')
    lines.append('# Eliashberg alpha^2F(omega) / lambda (isotropic).  Needs the harmonic fc2.')
    lines.append(
        'DO_ELIASHBERG = {}   # assemble g(k,q) and compute a2F / lambda'.format(
            bool(cfg['do_eliashberg'])
        )
    )
    lines.append(
        'SMEARING_EV = {}   # Fermi-surface double-delta Gaussian smearing (eV)'.format(
            cfg['smearing_ev']
        )
    )
    lines.append(
        'NK_ELECTRON = {}   # electronic k-grid (nk^3); None = native HRs grid. '
        'Must divide every supercell size.'.format(
            cfg['nk_electron'] if cfg['nk_electron'] else None
        )
    )
    ...
    lines.append('')
    lines.append('')
    lines.append('def _need_phonon_scf():')
    lines.append(
        '    """True when the fc2 must be computed here (Eliashberg, no reused forces)."""'
    )
    lines.append('    return DO_ELIASHBERG and not PHONON_FORCE_SETS')
    lines.append('')
    lines.append('')
    lines.append('def generate():')
    lines.append('    """Phase 1: write the displaced-supercell QE inputs."""')
    lines.append('    p = _make_paoflow()')
    lines.append('    p.electron_phonon(')
    lines.append('        supercell_matrix=SUPERCELL_MATRIX,')
    lines.append('        displacement_distance=DISPLACEMENT,')
    lines.append('        forces=None,')
    lines.append('        elphon_dir=ELPHON_DIR,')
    lines.append('        pp_dir=PP_DIR,')
    lines.append('        prefix=PREFIX,')
    lines.append('        hubbard_file=HUBBARD_FILE,')
    lines.append('        configuration=CONFIGURATION,')
    ...
    lines.append('')
    lines.append('')
    lines.append('def _run_inputs(indir, inputs):')
    lines.append('    """Run pw.x on each input (skipping finished ones)."""')
    lines.append('    for fin in inputs:')
    lines.append("        fout = fin[:-3] + '.out'")
    lines.append('        if _job_done(fout):')
    lines.append("            print('  skip (already done):', os.path.basename(fout))")
    lines.append('            continue')
    lines.append("        cmd = '{mpi} {pwx} -in {inp} > {out}'.format(")
    lines.append('            mpi=MPI_QE, pwx=QE_PW,')
    lines.append('            inp=os.path.basename(fin), out=os.path.basename(fout))')
    lines.append("        print('  running:', os.path.basename(fin))")
    lines.append('        subprocess.run(cmd, shell=True, check=True, cwd=indir)')
    lines.append('')
    ...
    lines.append('def run_supercells():')
    lines.append('    """Phase 2: run pw.x on every displaced-supercell input."""')
    lines.append('    indir = os.path.join(HERE, OUTPUTDIR, ELPHON_DIR)')
    lines.append("    inputs = sorted(glob.glob(os.path.join(indir, 'disp-*.in')))")
    lines.append("    inputs += sorted(glob.glob(os.path.join(indir, 'supercell.in')))")
    lines.append('    if not inputs:')
    lines.append(
        "        print('No e-ph inputs in {}; run the generate phase first.'.format(indir))"
    )
    lines.append('        sys.exit(1)')
    lines.append('    _run_inputs(indir, inputs)')
    lines.append('    if _need_phonon_scf():')
    lines.append('        pdir = os.path.join(HERE, OUTPUTDIR, PHONON_DIR)')
    lines.append("        pinputs = sorted(glob.glob(os.path.join(pdir, 'supercell-*.in')))")
    lines.append('        if not pinputs:')
    lines.append(
        "            print('No phonon inputs in {}; run the generate phase first.'.format("
    )
    lines.append('                pdir))')
    lines.append('            sys.exit(1)')
    lines.append('        _run_inputs(pdir, pinputs)')
    lines.append('')
    lines.append('')
    lines.append('def analyse():')
    lines.append('    """Phase 3: dV -> g_R -> (optional) Eliashberg a2F / lambda."""')
    lines.append('    p = _make_paoflow()')
    lines.append('    outdir = os.path.join(HERE, OUTPUTDIR, ELPHON_DIR)')
    lines.append('')
    lines.append('    if DO_ELIASHBERG:')
    lines.append(
        '        # Primitive PAO Hamiltonian: the electronic Bloch states of the coupling.'
    )
    lines.append('        p.projections(basispath=BASISPATH, configuration=CONFIGURATION)')
    lines.append('        p.projectability(pthr=PTHR)')
    lines.append('        p.pao_hamiltonian(shift_type=SHIFT_TYPE)')
    lines.append('')
    lines.append('    # dV = dH/du from the displaced supercells.')
    lines.append('    p.electron_phonon(')
    lines.append("        forces='qe',")
    lines.append('        supercell_matrix=SUPERCELL_MATRIX,')
    lines.append('        elphon_dir=ELPHON_DIR,')
    ...
    lines.append('        basispath=BASISPATH,')
    lines.append('        pthr=PTHR,')
    lines.append('        shift_type=SHIFT_TYPE,')
    lines.append('        project_good_subspace=PROJECT_GOOD_SUBSPACE,')
    lines.append('        displacement_mode=DISPLACEMENT_MODE,')
    lines.append('    )')
    lines.append('')
    lines.append('    # Fold to the primitive-cell real-space e-ph tensor g_R.')
    lines.append('    from PAOFLOW.elphon.do_gkq import assemble_eph_tensor')
    lines.append('    g = assemble_eph_tensor(')
    lines.append('        p.data_controller,')
    lines.append('        elphon_dir=ELPHON_DIR,')
    lines.append('        configuration=CONFIGURATION,')
    lines.append('        basispath=BASISPATH,')
    lines.append('        pthr=PTHR,')
    lines.append('        shift_type=SHIFT_TYPE,')
    lines.append('    )')
    lines.append("    np.save(os.path.join(outdir, 'g_R.npy'), g['g_R'])")
    lines.append("    if g.get('asr_residual') is not None:")
    lines.append("        print('  acoustic sum-rule residual: %.3e' % g['asr_residual'])")
    lines.append("    print('  wrote', os.path.join(outdir, 'g_R.npy'))")
    lines.append('')
    lines.append('    if not DO_ELIASHBERG:')
    lines.append('        return')
    lines.append('')
    lines.append('    # Harmonic fc2: reuse an existing FORCE_SETS, else harvest the phonon runs.')
    lines.append("    forces = PHONON_FORCE_SETS if PHONON_FORCE_SETS else 'qe'")
    lines.append('    p.phonons(')
    lines.append('        supercell_matrix=SUPERCELL_MATRIX,')
    lines.append('        displacement_distance=DISPLACEMENT,')
    lines.append('        forces=forces,')
    ...
    lines.append('        pp_dir=PP_DIR,')
    lines.append('        prefix=PREFIX,')
    lines.append('        do_bands=False,')
    lines.append('        do_dos=False,')
    lines.append('        do_thermal=False,')
    lines.append('    )')
    lines.append("    phonon = p.data_controller.data_arrays['phonopy']")
    lines.append('')
    lines.append('    # Isotropic Eliashberg alpha^2F(omega) and lambda.')
    lines.append('    from PAOFLOW.elphon.eph_kq import eliashberg')
    lines.append('    res = eliashberg(')
    lines.append('        p.data_controller,')
    lines.append("        g['g_R'],")
    lines.append('        phonon,')
    ...
    lines.append('def main():')
    lines.append('    parser = argparse.ArgumentParser(')
    lines.append("        description='PAOFLOW electron-phonon workflow.')")
    lines.append("    parser.add_argument('phase', nargs='?',")
    lines.append("                        choices=['generate', 'run', 'analyse', 'all'],")
    lines.append("                        default='all',")
    lines.append("                        help='Workflow phase to run (default: all).')")
...
def build_elphon_plot_script(cfg):
    """Assemble a plot.elphon.py that plots the Eliashberg alpha^2F(omega)."""
    lines = []
    lines.append('#!/usr/bin/env python3')
    lines.append('"""Plot the Eliashberg alpha^2F(omega) and cumulative lambda(omega).')
    lines.append('')
    lines.append('    python plot.elphon.py')
    lines.append('')
    lines.append('Reads output/elphon/eliashberg.npz written by main.elphon.py (analyse).')
    lines.append('"""')
    lines.append('')
    lines.append('import os')
    lines.append('')
    ...
    lines.append('HERE = os.path.dirname(os.path.abspath(__file__))')
    lines.append('OUTPUTDIR = {!r}'.format(cfg['outputdir']))
    lines.append("NPZ = os.path.join(HERE, OUTPUTDIR, 'elphon', 'eliashberg.npz')")
    lines.append('')
    lines.append('')
    lines.append('def main():')
    lines.append('    if not os.path.isfile(NPZ):')
    lines.append(
        "        raise SystemExit('%s not found; run main.elphon.py analyse first.' % NPZ)"
    )
    lines.append('    d = np.load(NPZ)')
    ...
    lines.append('        integrand = np.where(w > 0, 2.0 * a2F / w, 0.0)')
    lines.append(
        '    lam_cum = np.concatenate([[0.0], np.cumsum(0.5 * (integrand[1:] + integrand[:-1]) * np.diff(w))])'
    )
    lines.append('')
    lines.append('    fig, ax1 = plt.subplots(figsize=(6, 4))')
    lines.append("    ax1.plot(omega, a2F, color='C0', label=r'$\\alpha^2F(\\omega)$')")
    lines.append("    ax1.set_xlabel(r'$\\omega$ (meV)')")
    lines.append("    ax1.set_ylabel(r'$\\alpha^2F(\\omega)$', color='C0')")
    lines.append('    ax1.set_xlim(left=0.0)')
    lines.append('    ax1.set_ylim(bottom=0.0)')
    ...
    lines.append("    out = os.path.join(HERE, OUTPUTDIR, 'elphon', 'a2F.png')")
    lines.append('    fig.savefig(out, dpi=200)')
    lines.append("    print('wrote', out)")
    lines.append('')
    lines.append('')
    lines.append('if __name__ == "__main__":')
    lines.append('    main()')
...
def collect_elphon(common):
    """Prompt for the finite-difference electron-phonon configuration."""
    cfg = dict(common)
    cfg['supercell'] = ask_int('Supercell multiplicity (isotropic diagonal)', 2)
    cfg['displacement'] = ask_float('Displacement amplitude (Bohr)', 0.06)
    cfg['displacement_mode'] = ask_choice(
        'Displacement mode (symmetry = 1 displacement expanded by crystal symmetry; '
    ...
    cfg['pthr'] = ask_float('Projectability threshold', 0.95)
    cfg['shift_type'] = ask_int('PAO Hamiltonian shift scheme (shift_type)', 1)
    cfg['project_good_subspace'] = ask_yes_no(
        'Project dV onto the good-projectability subspace '
        '(removes the off-diagonal eta-completion artifact)?',
        True,
    )
    cfg['hubbard_file'] = ask(
        'pw.x input with a HUBBARD card to inject (on-site U only; blank = none)', ''
    ...
    cfg['pp_dir'] = ask('Pseudopotential directory (blank or "HERE" = script directory)', 'HERE')
    cfg['do_eliashberg'] = ask_yes_no(
        'Compute the isotropic Eliashberg alpha^2F(omega) / lambda '
        '(needs harmonic force constants)?',
        True,
    )
    if cfg['do_eliashberg']:
        cfg['smearing_ev'] = ask_float('  Fermi-surface double-delta Gaussian smearing (eV)', 0.30)
    ...
    cfg['plot'] = cfg['do_eliashberg'] and ask_yes_no(
        'Also generate a plotting script (plot.elphon.py)?', True
    )
    return cfg
