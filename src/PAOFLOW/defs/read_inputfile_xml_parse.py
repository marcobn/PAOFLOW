...
def read_inputfile_xml(fpath, inputfile, data_controller):
    ...
    # Control
    fpath = None
    restart = verbose = acbn0 = write2file = write_binary = writedata = use_cuda = writez2pack = (
        False
    )
    shift_type = 1
    shift = 'auto'
    ...
    Bfield = np.array(
        [[0, 0, 0]]
    )  # static magnetic firld placeholder: magnetic supercell not implemented!
    ...
    data_attributes['do_comparison'] = read_attribute(
        aroot, do_comparison, 'do_comparison', 'logical'
    )
    ...
    data_attributes['band_topology'] = read_attribute(
        aroot, band_topology, 'band_topology', 'logical'
    )
    data_attributes['eff_mass'] = read_attribute(aroot, eff_mass, 'eff_mass_topology', 'logical')
    data_attributes['do_spin_orbit'] = read_attribute(
        aroot, do_spin_orbit, 'do_spin_orbit', 'logical'
    )
    ...
    data_attributes['kramerskronig'] = read_attribute(
        aroot, kramerskronig, 'kramerskronig', 'logical'
    )
    data_attributes['critical_points'] = read_attribute(
        aroot, critical_points, 'critical_points', 'logical'
    )
    data_attributes['Berry'] = read_attribute(aroot, Berry, 'Berry', 'logical')
    data_attributes['ac_cond_Berry'] = read_attribute(
        aroot, ac_cond_Berry, 'ac_cond_Berry', 'logical'
    )
    ...
    data_arrays['high_sym_points'] = read_attribute(
        aroot, high_sym_points, 'high_sym_points', 'string_array'
    )
    ...
    data_attributes['symm_max_iter'] = read_attribute(
        aroot, symm_max_iter, 'symm_max_iter', 'integer'
    )
