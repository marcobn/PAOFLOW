...
def read_QE_output_xml(fpath):
    ...
    for event,elem in ET.iterparse(atomic_proj,events=('start','end')):
        ...
        if event == 'end':
            ...
            if 'ATMWFC' in elem.tag and readProj and group_nesting == 2 : #PROJECTIONS/K-POINT.{0:d}/ATMWFC.{1:d}
                ispin == 0
		#print "Reading", elem.tag, "spin component", ispin
		...
		#print "SPIN tag component", ispin, "k-point", ik
		for iin in range(nawf):
			wfc_type=elem.findall("ATMWFC.%d"%(iin+1))[0].attrib['type']
			aux     =elem.findall("ATMWFC.%d"%(iin+1))[0].text
                	aux = np.array(re.split(',|\n',aux.strip()),dtype='float32')
			if wfc_type=='real':
			    wfc = aux.reshape((nbnds,1))#wfc = nbnds x 1
			    U[:,iin,ik,ispin] = wfc[:,0]
