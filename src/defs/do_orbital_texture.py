...
# give the first and last band to calculate the orbital texture 
    for ib in range(attributes['ib_idx'],attributes['fb_idx'],1):
        ind_plot.append(ib)
        icount += 1
...
#        icount += 1
  ...
  for ik in range(snktot):
      for l in range(3):
        sktxtaux[ik,l,:,:] = np.conj(arrays['v_k'][ik,:,:,0].T).dot(Lj[l,:,:]).dot(arrays['v_k'][ik,:,:,0])
  ...
  if rank == 0:
    if 'kq' in arrays and E_k_full.shape[0] == arrays['kq'].shape[1]:
      ...
      for ib in range(icount):
        np.savez(os.path.join(attributes['opath'],'orbital_text_band_'+str(ib)), orbband=sktxt[:,:,:,:,ib])
