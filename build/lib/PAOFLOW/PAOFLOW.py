...
class PAOFLOW:
  ...
  def adhoc_spin_orbit ( self, naw=[1], phi=.0, theta=.0,  lambda_p=[.0], lambda_d=[.0], soc_strengh={}, soc_species=True ):
    ...
    if (soc_species==True):
      ...
      lambda_p=[]
      lambda_d=[]
      for i in range (len(arrays['atoms'])):
        lambda_p.append(soc_strengh[arrays['atoms'][i]][0])
        lambda_d.append(soc_strengh[arrays['atoms'][i]][1])
      arrays['lambda_p']=lambda_p
      arrays['lambda_d']=lambda_d
    else:
    ...
    naw=[]
    for i in range (len(arrays['atoms'])):
      if (arrays['shells'][arrays['atoms'][i]] == [0]):
        naw.append(1)
        orb.append('s')
      if (arrays['shells'][arrays['atoms'][i]] == [0,1]):
        naw.append(4)
        orb.append('sp')
      if (arrays['shells'][arrays['atoms'][i]] == [0,1,2]):
        naw.append(9)
        orb.append('spd')
      if (arrays['shells'][arrays['atoms'][i]] == [1,0]):
        naw.append(4)
        orb.append('ps')
      if (arrays['shells'][arrays['atoms'][i]] == [0,0,1,2]):
        naw.append(10)
        orb.append('sspd')
      if (arrays['shells'][arrays['atoms'][i]] == [0,0,1]):
        naw.append(5)
        orb.append('ssp')
      if (arrays['shells'][arrays['atoms'][i]] == [0,0,1,1,2]):
        naw.append(13)
        orb.append('ssppd')
    arrays['orb_pseudo'] = orb
    arrays['naw'] = naw
