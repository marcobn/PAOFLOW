...
Rust backend for [PAOFLOW](https://github.com/marcobn/PAOFLOW), importable as
`paoflow_rs`. The `acbn0` submodule provides the four-centre two-electron
Coulomb integrals (ERIs) used by the ACBN0 / eACBN0 modules.
...
- `acbn0_eri_batch(origins, prim_offsets, exps, coefs, norms, powers, keys)` —
  on-site `(ab|cd)` integrals over a single contracted-Gaussian basis.
- `acbn0_eri_batch_2c(...i_arrays..., ...j_arrays..., keys)` — intersite
  `(ik|jl)` integrals over two bases (atoms I and J) for eACBN0.
- `epsilon_eps_loop(...)` / `epsilon_jdos_loop(...)` — dielectric and JDOS inner
  BZ loops over the rank-local k-slice (Python fallback: vectorised NumPy in
...
1. `PAOFLOW_RS_THREADS` (legacy alias `PAOFLOW_ACBN0_THREADS`) — if set to a
   positive integer, a dedicated pool of that size is used. Set this to cap
   intra-rank threads and avoid oversubscription when running many MPI ranks
   per node (e.g. one thread per rank, or `cores_per_node / ranks_per_node`).
2. Otherwise the global `rayon` pool is used, which honours `RAYON_NUM_THREADS`
   and defaults to the number of logical cores.

`paoflow_rs.thread_count()` returns the effective thread count.
...
PAOFLOW_RS_THREADS=2 python scripts/bench.py
```
