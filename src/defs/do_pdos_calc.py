...
def do_pdos_calc(E_k,emin,emax,delta,netot,v_k,nk1,nk2,nk3,nawf,ispin):
    ...
    for m in range(nawf):
             ...
             E_kaux = np.zeros((nsize,nawf,ispin+1),dtype=float)
             ...
             for n in range (nsize):
                    for i in range(nawf):
                        pdosaux[i,:] += 1.0/np.sqrt(np.pi)*np.exp(-((ene[:]-E_kaux[n,m,ispin])/delta)**2)/delta*(np.abs(v_kaux[n,i,m,ispin])**2)
