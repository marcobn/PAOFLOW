...
def get_tau (temp,data_controller, channels ):
  ...
  h = cp.hbar
  kb = cp.Boltzmann
  hw0 = 0.063*1.6e-19 #joules
  rho = 2.329e3   #kg/m^3
  Eo = 41.45*1.6e-19  #joules
  a = 5.43e-10 #metres
  Ea = 5*1.6e-19 #joules
  v0 = 9.04e3    #m/s
  T  = 300.
  #T = np.linspace(300.,700.,50.)
  F_scr = 1. #1 for no screening. otherwise it has a value 
  ...
  bnd = attr['bnd']
  taus = []
  me = 0.19*9.11e-31*np.ones((snktot,bnd,nspin), dtype=float) #effective mass tensor in kg
  #E_actual =(1.6e-19*(arry['E_k'][:,:bnd])) #can i get rid of negative sign of energy for tay calcs?
  #E_actual =((arry['E_k'][:,:bnd])) #can i get rid of negative sign of energy for tay calcs?
  E_old =abs(1.6e-19*(arry['E_k'][:,:bnd])) #can i get rid of negative sign of energy for tay calcs?
  #print(np.min(E_old[E_old>1e-25]))
  #print(np.max(E_old[E_old<=1e-25]))
  #E_old[E_old>1e-25] -= (np.min(E_old[E_old>1e-25])+1e-35)
  #E_old[E_old<=1e-25] = np.max(E_old[E_old<=1e-25])+1e-35 - E_old[E_old<=1e-25]
  #E_modi =(1.6e-19*(arry['E_k'][:,:bnd]-np.min(arry['E_k'][:,:bnd])+1e-7)) #get rid of negative sign of energy by moving the fermi for tau calcs
  #E =abs(1.6e-19*(arry['E_k'][:,:bnd])) #can i get rid of negative sign of energy for tay calcs?
  #E_actual_re = np.reshape(E_actual,(snktot,bnd)) #i do this because i am not able to saave 3d arrays to a file
  E_old_re = np.reshape(E_old,(snktot,bnd)) #i do this because i am not able to saave 3d arrays to a file
  #E_modi_re = np.reshape(E_modi,(snktot,bnd)) #i do this because i am not able to saave 3d arrays to a file
  #di = ((1./di_inf)-(1./di_o))**-1   #dielectric constant
  for c in channels:
      if c == 'accoustic':
          
          #a_tau = (2*np.pi*h**4*rho*v0**2*np.power(E_old/(kb*T),(-0.5)))/((np.power(2*me*kb*T,1.5)*Ea**2))
          a_tau = (np.power(E_old,(-0.5)))
          taus.append(a_tau)

      if c == 'optical':
          o_tau = (2*(hw0/Eo)**2*h**2*a**2*rho*np.power(E_old/(kb*T),(-0.5)))/((np.power(2*me*kb*T,1.5))*np.pi)
          taus.append(o_tau)
 ...
 # np.savetxt('E_actual.dat',E_actual_re)
  np.savetxt('tau.dat',tau_new)
  np.savetxt('o_tau.dat',o_tau_new)
  np.savetxt('a_tau.dat',a_tau_new)
  np.savetxt('E.dat',E_old_re)
  #np.savetxt('E_modi.dat',E_modi_re)
  return(tau) 
