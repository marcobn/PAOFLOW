...
SAVEDIR = os.path.join(HERE, 'Al1.save')
UPFS = [os.path.join(HERE, 'Al1.save/Al.upf')]
...
IBRAV = 2
NK = 891   # k-points along the band path
...
EMIN, EMAX, NE = -11.0, 10.1, 1000
DO_PDOS = False
TMIN, TMAX, NT = 300.0, 300.0, 1   # temperature grid (K) for transport
...
def run_properties():
    ...
    p.transport(tmin=TMIN, tmax=TMAX, nt=NT, emin=EMIN, emax=EMAX,
                ne=NE, write_to_file=True)

    p.finish_execution()


def run_optical():
    """Optical properties: extended basis + non-local velocity.

    The dielectric tensor requires the non-local velocity correction,
    so it runs as a separate PAOFLOW instance on the extended basis.
    """
    p = PAOFLOW.PAOFLOW(
        workpath=HERE,
        outputdir=OUTPUTDIR,
    ...
    p.projections(basispath=BASISPATH, configuration='extended')
    p.projectability(pthr=PTHR)
    p.pao_hamiltonian()
    if NFFT is not None:
        p.interpolated_hamiltonian(nfft1=NFFT[0], nfft2=NFFT[1], nfft3=NFFT[2])
    ...
    # Non-local velocity is essential for the optical matrix elements.
    p.gradient_and_momenta(nonlocal_velocity=True)
    p.adaptive_smearing()
