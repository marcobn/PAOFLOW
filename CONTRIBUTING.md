...
In order to run the tests, you will need to have `pytest` installed in your environment. You can install it using pip:
...
To run all tests, simply run the following command from the repository root:

```bash
pytest -q tests
```

If one wants to run specific files or folders:
...
To run a unit test for a specific function:
