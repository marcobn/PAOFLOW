...
def do_transport ( data_controller, temps, ene, velkp ):
  ...
  fkappa = ojf('kappa')
  fsigma = ojf('sigma')
  fSeebeck = ojf('Seebeck')
  fsigmadk = ojf('sigmadk') if attr['smearing']!=None else None
  ...
  for temp in temps:
    ...
    if attr['smearing'] != None:
      L0 = do_Boltz_tensors_smearing(data_controller, itemp, ene, velkp)
    ...
    L0,L1,L2 = do_Boltz_tensors_no_smearing(data_controller, itemp, ene, velkp)
