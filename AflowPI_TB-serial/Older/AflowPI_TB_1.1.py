...
nkpi=500
kx=np.linspace(0.0, 1.0, nkpi)
ky=np.zeros(nkpi)
kz=np.zeros(nkpi)
kp=np.array([kx,ky,kz])
kq=np.zeros((3,nkpi),dtype=float)
for ik in range(nkpi):
	kq[:,ik] = kx[ik]*b_vectors[0,:]+ky[ik]*b_vectors[1,:]+kz[ik]*b_vectors[2,:]
kmod = np.zeros((nkpi),dtype=float)
