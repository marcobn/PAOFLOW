...
def read_config_in(path: Union[str, Path] = "config.in") -> SkeafConfig:
    ...
    if len(lines) < 15:
        raise ValueError(
            f"{path}: expected at least 15 lines, got {len(lines)}.  "
            "Is this a SKEAF config.in?"
        )
...
def write_config_in(cfg: SkeafConfig, path: Union[str, Path] = "config.in") -> None:
    ...
    with path.open("w") as fh:
        ...
        fh.write(_line(f"{cfg.freq_same_frac:7.3f}",           "Maximum fractional diff. between orbit freqs. for averaging"))
        fh.write(_line(f"{cfg.avg_same_frac:7.3f}",            "Maximum distance between orbit avg. coords. for averaging"))
