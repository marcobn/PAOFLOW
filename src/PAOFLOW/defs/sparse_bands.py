...
def _dd_value(da, db, lx, ly, lz, h):
    ...
    if da == db == 'dx2-y2':
        return 0.75 * diff**2 * dds + (l2 + m2 - diff**2) * ddp + (n2 + 0.25 * diff**2) * ddd
    if da == db == 'dz2':
        t = n2 - 0.5 * (l2 + m2)
    ...
    if pair == frozenset(('dxy', 'dyz')):
        return 3.0 * lx * m2 * lz * dds + ln * (1.0 - 4.0 * m2) * ddp + ln * (m2 - 1.0) * ddd
    if pair == frozenset(('dxy', 'dzx')):
        return 3.0 * l2 * ly * lz * dds + mn * (1.0 - 4.0 * l2) * ddp + mn * (l2 - 1.0) * ddd
    if pair == frozenset(('dyz', 'dzx')):
        return 3.0 * ly * n2 * lx * dds + lm * (1.0 - 4.0 * n2) * ddp + lm * (n2 - 1.0) * ddd
    if pair == frozenset(('dxy', 'dx2-y2')):
        return 1.5 * lm * diff * dds + 2.0 * lm * (m2 - l2) * ddp + 0.5 * lm * diff * ddd
    if pair == frozenset(('dyz', 'dx2-y2')):
        return 1.5 * mn * diff * dds - mn * (1.0 + 2.0 * diff) * ddp + mn * (1.0 + 0.5 * diff) * ddd
    if pair == frozenset(('dzx', 'dx2-y2')):
        return 1.5 * ln * diff * dds + ln * (1.0 - 2.0 * diff) * ddp - ln * (1.0 - 0.5 * diff) * ddd
    if pair == frozenset(('dxy', 'dz2')):
        t = n2 - 0.5 * (l2 + m2)
        return _SQRT3 * (lm * t * dds - 2.0 * lm * n2 * ddp + 0.5 * lm * (1.0 + n2) * ddd)
    if pair == frozenset(('dyz', 'dz2')):
        t = n2 - 0.5 * (l2 + m2)
        return _SQRT3 * (mn * t * dds + mn * (l2 + m2 - n2) * ddp - 0.5 * mn * (l2 + m2) * ddd)
    if pair == frozenset(('dzx', 'dz2')):
        t = n2 - 0.5 * (l2 + m2)
        return _SQRT3 * (ln * t * dds + ln * (l2 + m2 - n2) * ddp - 0.5 * ln * (l2 + m2) * ddd)
...
def _sk_element(orb_a, orb_b, lx, ly, lz, h):
    ...
    if orb_a in _P_INDEX and orb_b in _P_INDEX:
        ...
        return (lx, ly, lz)[_P_INDEX[orb_a]] * (lx, ly, lz)[_P_INDEX[orb_b]] * (pps - ppp)
    return 0.0
...
def _sk_block_spd_vec(lx, ly, lz, hop):
    ...
    H[:, 7, 7] = 0.75 * diff**2 * dds + (l2 + m2 - diff**2) * ddp + (n2 + 0.25 * diff**2) * ddd
    H[:, 8, 8] = t_dz2**2 * dds + 3.0 * n2 * (l2 + m2) * ddp + 0.75 * (l2 + m2) ** 2 * ddd

    # ── d-d off-diagonal (upper triangle, then symmetrise) ──
    H[:, 4, 5] = 3.0 * lx * m2 * lz * dds + ln * (1.0 - 4.0 * m2) * ddp + ln * (m2 - 1.0) * ddd
    H[:, 4, 6] = 3.0 * l2 * ly * lz * dds + mn * (1.0 - 4.0 * l2) * ddp + mn * (l2 - 1.0) * ddd
    H[:, 5, 6] = 3.0 * ly * n2 * lx * dds + lm * (1.0 - 4.0 * n2) * ddp + lm * (n2 - 1.0) * ddd
    H[:, 4, 7] = 1.5 * lm * diff * dds + 2.0 * lm * (m2 - l2) * ddp + 0.5 * lm * diff * ddd
    H[:, 5, 7] = (
        1.5 * mn * diff * dds - mn * (1.0 + 2.0 * diff) * ddp + mn * (1.0 + 0.5 * diff) * ddd
    )
    H[:, 6, 7] = (
        1.5 * ln * diff * dds + ln * (1.0 - 2.0 * diff) * ddp - ln * (1.0 - 0.5 * diff) * ddd
    )
    H[:, 4, 8] = _SQRT3 * (lm * t_dz2 * dds - 2.0 * lm * n2 * ddp + 0.5 * lm * (1.0 + n2) * ddd)
    H[:, 5, 8] = _SQRT3 * (
...
class SparseEDTB:
    ...
    def _parse_model(self, md):
        ...
        self.b_vectors[0] = 2.0 * np.pi * np.cross(self.a_vectors[1], self.a_vectors[2]) / vol
        self.b_vectors[1] = 2.0 * np.pi * np.cross(self.a_vectors[2], self.a_vectors[0]) / vol
        self.b_vectors[2] = 2.0 * np.pi * np.cross(self.a_vectors[0], self.a_vectors[1]) / vol
        ...
        for ia in range(1, self.natoms):
            self.atom_block_start[ia] = self.atom_block_start[ia - 1] + self.norbitals[ia - 1]
        ...
        for i in range(-cr[0], cr[0] + 1):
            for j in range(-cr[1], cr[1] + 1):
                for k in range(-cr[2], cr[2] + 1):
                    R_vec = i * self.a_vectors[0] + j * self.a_vectors[1] + k * self.a_vectors[2]
        ...
        if _use_kdtree:
            ...
            for s in range(0, n_sc, _BATCH):
                ...
                for local_i, nlist in enumerate(neigh_lists):
                    ...
                    dists_i = np.linalg.norm(sctau_flat[nlist] - sctau_flat[global_i], axis=1)
            ...
            f_c_sc_sparse = csr_matrix((fc_sc_vals, (fc_sc_rows, fc_sc_cols)), shape=(n_sc, n_sc))
            ...
            for ii in range(-cr[0], cr[0] + 1):
                for jj in range(-cr[1], cr[1] + 1):
                    for kk in range(-cr[2], cr[2] + 1):
                        R_vec = (
                            ii * self.a_vectors[0] + jj * self.a_vectors[1] + kk * self.a_vectors[2]
                        )
                        ...
                        # Screening
                        R_offset = ((ii + cr[0]) * int(nk2) + (jj + cr[1])) * int(nk3) + (
                            kk + cr[2]
                        )
    ...
    def build_hk(self, k_frac):
        ...
        n_onsite = self.nawf
        all_rows = np.concatenate([self._bond_rows, np.arange(n_onsite, dtype=np.int32)])
        all_cols = np.concatenate([self._bond_cols, np.arange(n_onsite, dtype=np.int32)])
