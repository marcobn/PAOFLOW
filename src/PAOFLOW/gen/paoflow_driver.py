...
4. **Electron-phonon** (``main.elphon.py``) -- the atomic-orbital (Agapito &
...
# --------------------------------------------------------------------------- #
# Electron-phonon (AO route) workflow builder
# --------------------------------------------------------------------------- #
ELPHON_TEMPLATE = r'''#!/usr/bin/env python3
"""PAOFLOW electron-phonon (Eliashberg) workflow -- AO route (generated by paoflow_gen.py).

Atomic-orbital (Agapito & Bernardi, Phys. Rev. B 97, 235146 (2018)) interpolation
...
Two phases:
    ...
    python main.elphon.py analyse   # AO interpolation -> alpha^2F, lambda, Tc
...
def analyse():
    """Phase 2: AO interpolation of the QE coupling -> alpha^2F, lambda, Tc."""
    ...
    print('AO-route Eliashberg (%s, source=%s, Nk=%d):' % (PREFIX, SOURCE, NK_DENSE))
...
def main():
    parser = argparse.ArgumentParser(description='PAOFLOW electron-phonon (AO) workflow.')
...
def build_elphon_script(cfg):
    """Assemble a main.elphon.py electron-phonon (AO route) workflow from the config."""
    subs = {
...
def collect_elphon(common):
    """Prompt for the electron-phonon (AO route) configuration."""
    cfg = dict(common)
