...
def read_QE_output_xml(fpath,verbose,non_ortho):
    ...
    group_nesting = 0
    readEigVals = False; readProj = False; spincalc = False
    for event,elem in ET.iterparse(atomic_proj,events=('start','end')):
        ...
        if event == 'end':
		    ...
		    if verbose: print("Reading", subelem.tag,"of k-point", ik)
			...
			subelem = elem.findall("EIG.%d"%(ispin+1))[0]
		    	if verbose: print("Reading", subelem.tag,"spin component", ispin, "of k-point", ik)
                        eigk_type=subelem.attrib['type']
                        eigk_file=np.array(subelem.text.split(),dtype='float32')
                        my_eigsmat[:,ik,ispin] = np.real(eigk_file)*Ry2eV-Efermi #meigs in eVs and wrt Ef
            ...
            if 'ATMWFC' in elem.tag and readProj and nspin == 1: #PROJECTIONS/K-POINT.{0:d}/ATMWFC.{1:d}
                ispin == nspin-1
		if verbose:print("Reading", elem.tag, "of non-spin polarized calc") 
		...
		for iin in range(nawf):
			...
			if verbose:print("Reading",subelem.tag, "SPIN component", ispin, "k-point", ik)
