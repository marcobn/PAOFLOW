...
def get_exeCmd(prefix, engine, calcType,inputFile):
    ...
    execPostfix = " " 
    QE_path = "/home/hw0163/codes/qe-6.1.sca/qe-6.1/bin"
    PAO_path = "mpirun -np 32 python /storage/scratch2/hw0163/GitHub/PAOFLOW/src/"
...
def oneRun(prefix,scfOne=False,isInit=False,startPt='scf'):
		...
		#Run scf,nscf,pdos and PAO_bands
		if isInit:
			newList = calcList[calcList.index(startPt):]
			calcList = newList
		else:
			calcList = fullList
...
def main():
	...
	if len(sys.argv) == 2:
		...
		prefix = sys.argv[1]
		startPt = sys.argv[2]
		newUvals = oneRun(prefix, isInit=True, startPt=startPt)
		convergence = False
		while convergence == False:
