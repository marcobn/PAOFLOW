...
def graphene( data_controller ):
  ...
  # H00
  arry['HRs'][0,1,0,0,0,0]=attr['t']
  arry['HRs'][1,0,0,0,0,0]=attr['t']

  # H10
  arry['HRs'][1,0,1,0,0,0]=attr['t']

  #H20
  arry['HRs'][:,:,2,0,0,0]=np.conj(arry['HRs'][:,:,1,0,0,0]).T

  #H01
  arry['HRs'][1,0,0,1,0,0]=attr['t']
  ...
  #arry['a_vectors'] = np.array([[(3**.5)/2.0, 0.5, 0], [(3**.5)/2.0, -0.5, 0], [0, 0, 10]])
  arry['a_vectors'] = arry['a_vectors']/ANGSTROM_AU

  # Atomic coordinates
  arry['tau'] = np.zeros((2,3),dtype=float) 
#  arry['tau'][0,0] = (3**.5)/3
#  arry['tau'][1,0] = 2*(3**.5)/3
  ...
  # Reciprocal Lattice
  b_vectors = np.zeros((3,3),dtype=float)
