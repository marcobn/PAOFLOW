...
def get_tau (temp,data_controller, channels ):
  ...
  E =abs(1.602e-19*(arry['E_k'][:,:bnd])) #can i get rid of negative sign of energy for tay calcs?
  ...
  for c in channels:
      if c == 'accoustic':
          ...
          a_tau = (2*np.pi*h**4*rho*v0**2*np.power(E/(kb*temp),(-0.5)))/((np.power(2*me*kb*temp,1.5)*Ea**2))
          taus.append(a_tau)

      if c == 'optical':
          o_tau = (2*(hw0/Eo)**2*h**2*a**2*rho*np.power(E/(kb*temp),(-0.5)))/((np.power(2*me*kb*temp,1.5))*np.pi)
          taus.append(o_tau)
