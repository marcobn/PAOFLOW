...
# Define paths
sys.path.append('./')
sys.path.append('/Users/Haihang/Dropbox/AflowPI_TB-serial/src/defs')
...
from do_pdos_calc import do_pdos_calc
...
read_S, shift_type, fpath, shift, pthr, do_comparison, double_grid,\
	do_bands, onedim, do_dos, do_pdos, delta, nfft1, nfft2, nfft3 = read_input(input_file)
...
if do_pdos:
	# PDOS calculation with gaussian smearing on double_grid Hksp
        do_pdos_calc(Hksp,Sksp,read_S,shift,delta)

        print('pdos in ',time.clock()-reset,' sec')
        reset=time.clock()
