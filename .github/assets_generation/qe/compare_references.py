...
def _compare_numeric(
    a_vals: list[float], b_vals: list[float], *, atol: float, rtol: float
...
def _compare_file(ref_file: Path, cand_file: Path, *, atol: float, rtol: float) -> FileComparison:
    ...
    if not cand_file.exists():
        return FileComparison(
            relpath=relpath, status='missing_candidate', reason='candidate file missing'
        )
    ...
    if ref_text is None or cand_text is None:
        ...
        return FileComparison(
            relpath=relpath, status='different_binary', reason='binary files differ'
        )
...
def compare_job(
    job_dir: Path, baseline_dirname: str, candidate_dirname: str, *, atol: float, rtol: float
) -> dict[str, Any]:
    ...
    for rel in all_paths:
        ...
        cmp_result = _compare_file(
            ref_file, cand_file if cand_file is not None else candidate / rel, atol=atol, rtol=rtol
        )
        rec = {
            'relpath': rel,
        ...
        if cmp_result.max_abs_diff is not None:
            result['summary']['max_abs_diff'] = max(
                result['summary']['max_abs_diff'], cmp_result.max_abs_diff
            )
        if cmp_result.max_rel_diff is not None:
            result['summary']['max_rel_diff'] = max(
                result['summary']['max_rel_diff'], cmp_result.max_rel_diff
            )

    if (
        result['summary']['mismatch']
        or result['summary']['missing_candidate']
        or result['summary']['extra_candidate']
    ):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Compare PAOFLOW Reference folders across examples/jobs.'
    )
