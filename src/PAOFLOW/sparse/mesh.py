...
def run_mesh(
    data_controller, sparse_h, nev, consumers=(), afac=None, smearing='gauss', verbose=False
):
    ...
    for ispin in range(nspin):
        ...
        for ik in range(nk_local):
            ...
            E, V = solve_lowest(hk, nev, v0=v0)
            v0 = np.ascontiguousarray(V[:, 0])

            vel = np.empty((3, nev), dtype=float)
            for l in range(3):
                W = dhk[l] @ V  # (nawf, nev)
                vel[l] = np.einsum('an,an->n', np.conj(V), W).real
