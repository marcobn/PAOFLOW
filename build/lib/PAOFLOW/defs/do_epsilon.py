...
def eps_loop ( data_controller, ene, ispin, ipol, jpol):
  ...
  from .smearing2 import Gaussian,FermiDirac,MethPaxton,ColdSmearing
### What is this?
  ...
  degauss = 0.05
  if smearing == None:
    fn = FermiDirac(arrays['E_k'][:,:bnd,ispin],Ef,degauss)
#    fn = 2.*1./(1.+np.exp(arrays['E_k'][:,:bnd,ispin]/temp))
  elif smearing == 'gauss':
    fn = Gaussian(arrays['E_k'][:,:bnd,ispin], Ef, degauss)
#    fn = 2.*intgaussian(arrays['E_k'][:,:bnd,ispin], Ef, arrays['deltakp'][:,:bnd,ispin])
  elif smearing == 'm-p':
    fn = MethPaxton(arrays['E_k'][:,:bnd,ispin], Ef, degauss)
