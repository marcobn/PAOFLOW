...
def paoflow(inputpath='./',inputfile='inputfile.xml'):
    ...
    try:
        ...
        high_sym_points,tmin,tmax,tstep = read_inputfile_xml(inputpath,inputfile)
            ...
            for t in range(temps.shape[0]):
                ...
                for ispin in range(nspin):
                    ...
                    if rank == 0:
                        ...
                        for n in range(ene.size):
                            kappa[:,:,n] = (L2[:,:,n] - temp*11604.52500617*np.dot(L1[:,:,n],np.dot(LAN.inv(L0[:,:,n]),L1[:,:,n])))*1.e6
                        for n in range(ene.size):
