    ...
    for n in range(s_tensor.shape[0]):
        ...
        if rank == 0:
            if twoD:
                ...
                cgs_conv = 1.0 / (
                    np.linalg.norm(np.cross(av0, av1)) * attr["alat"] ** 2
                )
            else:
                cgs_conv = (
                    1.0e8
        ...
        if do_ac:
            ...
            ene, sigxy = do_ac_conductivity(
                data_controller, jksp_js, pksp_i, ipol, jpol
            )
            if rank == 0:
                sigxy *= cgs_conv
...
def do_Berry_curvature(data_controller, jksp, pksp):
    ...
    for ik in range(snktot):
        E_nm = (
            arrays["E_k"][ik, :, 0] - arrays["E_k"][ik, :, 0][:, None]
        ) ** 2 + deltap**2
    ...
    if attributes["shift"] != 0.0:
        attributes["emaxH"] = np.amin(
            np.array([attributes["shift"], attributes["emaxH"]])
        )
    ### Hardcoded 'de'
        ...
        if attributes["smearing"] == "gauss":
            Om_zkaux[:, i] = np.sum(
                (
                    Om_znkaux[:, :]
                    * intgaussian(
                        arrays["E_k"][:, :, 0], ene[i], arrays["deltakp"][:, :, 0]
                    )
                ),
                axis=1,
            )
            ...
            Om_zkaux[:, i] = np.sum(
                Om_znkaux[:, :]
                * (0.5 * (-np.sign(arrays["E_k"][:, :, 0] - ene[i]) + 1)),
                axis=1,
            )
...
def smear_sigma_loop(data_controller, ene, pksp_i, pksp_j, ispin, ipol, jpol):
    ...
    for n in range(nawf):
        for m in range(nawf):
            if m != n:
                E_diff_nm[:, n, m] = (
                    arry["E_k"][:, n, ispin] - arry["E_k"][:, m, ispin]
                ) ** 2
    ...
    for e in range(esize):
        if attr["smearing"] != None:
            ...
            sigxy[e] = np.sum(
                f_nm[:, :, :]
                / (E_diff_nm[:, :, :] - (ene[e] + 1.0j * arry["delta"]) ** 2 + eps)
            )
