...
def get_tau ( data_controller, channels ):
  ...
  T = 300
  taus = []
  me = 0.19*np.ones((snktot,nawf,nspin), dtype=float) #effective mass tensor
  for c in channels:
      if c == 'accoustic':
      ...
      tau = np.zeros((snktot,bnd), dtype=float)
      for t in taus:
        tau += 1./t
  return(tau) 
...
def L_loop ( data_controller, temp, smearing, ene, velkp, t_tensor, alpha ):
  ...
  taus = get_tau(arrays['E_k'][1])
  for n in range(bnd):
    Eaux = np.reshape(np.repeat(arrays['E_k'][:,n,0],esize), (snktot,esize))
    taus = np.reshape(np.repeat(arrays['E_k'][:,n,0],esize), (snktot,esize))
    ...
    for l in range(t_tensor.shape[0]):
      ...
      L[i,j,:] += np.sum(kq_wght*velkp[:,i,n]*taus[:,n]*velkp[:,j,n]*(smearA*EtoAlpha).T, axis=1)
  print(L)     
  return L
