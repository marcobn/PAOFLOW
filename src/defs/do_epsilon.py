...
def do_epsilon(E_k,pksp,kq_wght,delta,temp,ispin):
    ...
    epsi_aux[:,:,:,0] = epsi_loop(ini_ik,end_ik,ene,E_k,pksp,kq_wght,delta,temp,ispin)
...
def epsi_loop(ini_ik,end_ik,ene,E_k,pksp,kq_wght,delta,temp,ispin):
        ...
        for n in range(pksp.shape[1]):
            ...
            for m in range(pksp.shape[1]):
                ...
                if n != m:
                    for i in range(3):
                        for j in range(3):
                            epsi[i,j,:] += (ene[:]**2+delta**2)/(ene[:]**2+delta**2)**2 * \
