...
def do_transport ( data_controller, temps, ene, velkp, carrier_conc, save_L0, channels, weights, write_to_file, fit):
  import numpy as np
  ...
  if rank == 0:
    ...
    arrays['S'] = np.empty((3,3), dtype=float)
    arrays['kappa'] = np.empty((3,3), dtype=float)
  ...
  for ispin in range(nspin):
    ...
    for temp in temps:
      ...
      if attr['smearing'] is not None and not fit:
