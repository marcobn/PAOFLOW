"""eACBN0 joint U+V SCF run on diamond Si.

Runs three calculations in sequence on the same Si primitive cell and
compares their band structures on a single plot:

  1. **bare**   — plain DFT, no Hubbard correction.
  2. **U-only** — ACBN0 self-consistent U on Si-3s and Si-3p.
  3. **U+V**    — eACBN0 self-consistent U on the same orbitals plus
                  intersite V on every Si-Si bond within 2.6 Å
                  (the 4 nearest neighbours at d = a*sqrt(3)/4 ~ 2.43 Å).
...
def compute_bands(label):
    """Build the PAO Hamiltonian from the current ``Si.save`` and dump
...
# Si 3s is too delocalized for a meaningful on-site U; keep the s
# manifold active so the V_ss, V_sp channels survive, but set its U to
# zero.  Re-run DFT once with the updated HUBBARD card before bands.
e.uVals['Si-3s'] = 0.0
