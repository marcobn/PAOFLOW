...
if do_dos or Boltzmann or epsilon or Berry:
    ...
    for ispin in range(nspin):
        eig, E_k, v_k,v_kall = calc_TB_eigs_vecs(Hksp,ispin,npool)
    if rank == 0:
        Hksp = np.reshape(Hksp,(nk1,nk2,nk3,nawf,nawf,nspin),order='C')
...
if do_pdos:
    ...
    if nspin == 1 or nspin == 2: 
        if rank == 0: eigup = eig[:,0]
        do_dos_calc(eigup,emin,emax,delta,eigtot,v_kall,nk1,nk2,nk3,nawf,0)
        eigup = None
    if nspin == 2:
        if rank == 0: eigdw = eig[:,1]
        do_dos_calc(eigdw,emin,emax,delta,eigtot,v_kall,nk1,nk2,nk3,nawf,1)
        eigdw = None
